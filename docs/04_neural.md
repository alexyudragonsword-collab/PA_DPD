...
### 5.1b Phase 2.5:长帧(F=200)与 TCN

| 数据集 | GMP-510 | DGRU F=200 | **TCN-H16 F=200** | OpenDPD 参考 |
|--------|---------|-----------|-------------------|--------------|
| DPA_200MHz | -33.7 | -31.9 | **-34.9(464 参数)** | — |
| APA_200MHz | -35.5 | -31.6 | — | GRU-H23 -43.5 |
| DPA_160MHz(stride=8) | -39.2 | -37.3 | — | GRU-H24 -38.4 |

**TCN-H16 以 -34.9 dB 首次超过经典 GMP-510(-33.7)**——最利于 ASIC
部署的膨胀卷积结构胜出,直接对接 Phase 3。长帧(F=200)相对 F=50 在
DPA/APA 上改善甚微:两数据集记忆跨度本就短,非帧长受限。**APA 的 -43.5
仍无法复现**:系统排查学习率(1e-3/5e-3/5e-4)、backbone(GRU/DGRU)、
帧长(50/200)、数据集变体(A/_b,GMP 在 _b 上更差 -29.3)四类变量,
全部收敛在 ≈-31.5 平台,而同一流水线在 DPA_160 上与其发表值仅差 1 dB,
证明实现无误。判定为"以公开信息无法复现",不影响 §5.3 的 DPD 结论。

### 5.2 DPD 线性化(OpenDPD 口径,测试集,DLA 神经 DPD)

| 数据集 | 无 DPD | DLA DGRU-H8(486 参数) | EVM(谱) | OpenDPD 发表 |
|--------|--------|------------------------|---------|--------------|
| DPA_200MHz | -30.6 | **-49.5** | -44.2 | — |
| DPA_160MHz | -34.5 | **-53.1** | -49.1 | GRU -51.9 / TRes-DeltaGRU -56.8 |

**DPA_160MHz DLA DPD -53.1 dBc,越过 -52 验收线并优于其发表 GRU(-51.9)**,
逼近其最优 TRes-DeltaGRU(-56.8;delta 稀疏 RNN 因 CPU 训练不可行推迟)。
评估代理为对应数据集的 F=200 神经模型。
...
| **神经 DGRU-H23(F=200)** | -31.6 | **-38.56** | **-38.81** |
...
- 代理评估的读数只在代理 NMSE 之上可信;强非线性 PA(APA/GaN)
  必须用神经代理评估 DPD。
- Phase 2.5 已完成:
  1. ✅ frame_length=200 训练——证实 DPA/APA 记忆跨度短,非帧长受限;
     APA -43.5 经四类变量排查判定为公开信息无法复现
  2. ✅ TCN backbone——**-34.9 dB 超过 GMP-510**,ASIC 友好,对接 Phase 3
  3. ✅ DLA 神经 DPD 真实数据:DPA_160 -53.1 dBc 过验收线
- 待续(需 GPU 或更大算力预算):
  1. DeltaGRU/TRes-DeltaGRU(OpenDPD v2 最优 -56.8;delta 稀疏 RNN 为
     Python 循环实现,CPU BPTT 慢 10~50×,240 epoch 不可行)
  2. lr=5e-3 + 240 epochs(OpenDPDv2 配方)、多 seed 统计
  3. APA -43.5 深究(需复现其确切训练细节)
  4. Transformer 探索(320 MHz 长记忆)

**本环境限制记录**:4 核 CPU、无 GPU;长时后台训练任务熬不过容器空闲
挂起(Phase 2.5 训练链两次在 ~1.5-2h 处被回收),故采用"单步快跑、
完成即提交"策略。完整多 seed / 长训练建议在 GPU 环境用本仓库脚本运行。
