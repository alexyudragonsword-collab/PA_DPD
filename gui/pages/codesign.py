...
ui.page_setup("PA/DPD 联合设计", "🧭")
ui.get_state()
ui.note("AI-Native 流程:PA 工作点与 DPD 复杂度联合优化,而非"
        "\"先设计 PA 再补救线性度\"。左:离散 Pareto 扫描(稳健);"
        "右:可微梯度寻优(快速定位,逼近可逆壁垒时会振荡)。")

tab1, tab2 = st.tabs(["📊 离散 Pareto 扫描", "∇ 可微梯度寻优"])

with tab1:
    c1, c2, c3 = st.columns(3)
    evm_spec = c1.slider("EVM spec (dB)", -50, -30, -40, key="spec1")
    budget = c2.slider("DPD 系数预算", 20, 200, 90)
    bw = c3.select_slider("带宽 (MHz)", [20, 80, 160], 80, key="bw1") * 1e6
    if st.button("运行扫描", type="primary"):
        ...
        with st.spinner("扫描 7 个工作点(每点含 DPD 阶梯搜索)…"):
    ...
    if "cod_rows" in st.session_state:
        ...
        c1.metric("顺序设计(先冲效率)",
                  ...
                  "可行" if seq["feasible"] and seq["dpd_cost"] <= budget
                  else "撞墙:不可逆/超预算", delta_color="off")
        if feasible:
            co = max(feasible, key=lambda r: r["pae"])
            c2.metric("联合设计(预算内最高效率)",
                      f"PAE {100*co['pae']:.1f}%",
                      f"drive {co['drive']:.2f} · {co['dpd_cost']} 系数 · "
                      f"EVM {co['evm_dpd']:.1f}", delta_color="off")
        st.dataframe([{"drive": r["drive"], "PAE %": f"{100*r['pae']:.1f}",
                       "EVM 无DPD": f"{r['evm_nodpd']:.1f}",
                       "DPD 系数": r["dpd_cost"],
                       "EVM DPD": f"{r['evm_dpd']:.1f}",
                       "可行": "✅" if r["feasible"] else "❌"}
                      for r in rows],
...
with tab2:
    ...
    drive0 = c2.slider("初始 drive(保守)", 0.06, 0.14, 0.08, 0.01)
    steps = c3.slider("梯度步数", 50, 300, 150)
    if st.button("运行梯度寻优", type="primary"):
        try:
            from padpd.codesign_torch import joint_codesign
            from padpd.waveform import OFDMConfig, generate_ofdm
            with st.spinner("内层 LS-DPD + 外层梯度优化中…"):
