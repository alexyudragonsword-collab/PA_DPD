...
class DataPage(QWidget):
    def __init__(self):
        ...
        self.btn_load_ds = QPushButton("加载数据集")
        self.btn_load_ds.setObjectName("primary")
        self.btn_file = QPushButton("打开文件 (CSV/.mat/.npz)…")
        self.align = QCheckBox("自动延迟对齐")
        gl.addWidget(QLabel("OpenDPD 目录"))
        ...
        sel_row = QHBoxLayout()
        sel_row.addWidget(QLabel("已注册数据源"))
        self.sel = QComboBox()
        self.btn_rm = QPushButton("移除")
        ...
        self.c_bw = MetricCard("主带宽")
        self.c_mod = MetricCard("调制 / 子信道")
    ...
    def load_file(self):
        ...
        try:
            ...
            if src.get("align_info"):
                self.msg.setText(f"对齐:延迟 "
                                 f"{src['align_info']['lag_total']:.2f} 采样")
        except Exception as e:
            self.msg.setText(f"❌ 加载失败:{e}")
    ...
    def _register(self, src):
        ...
        self.msg.setText(f"✅ 已注册:{src['name']}")
    ...
    def preview(self, name):
        ...
        self.p_psd.set_figure(figs.psd_fig(
            {"PA 输入": src["x_train"][:n], "PA 输出": src["y_train"][:n]},
            src["fs"]))
