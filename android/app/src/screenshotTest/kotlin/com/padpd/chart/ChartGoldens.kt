 ...
 *
 * The JSON is compiled in (see ChartFixtures.kt, generated by
 * tests/test_chart_fixtures.py) because at render time there is no file
 * to read. The screenshot plugin runs each preview inside layoutlib, in
 * its own process, and neither route survives that: the screenshotTest
 * source set's java resources are not on the render classpath, and the
 * AssetManager the preview is handed does not serve the module's assets
 * either. Both were tried on the device that matters - CI - and the
 * second reported `assets=true, classpath=false`, an AssetManager that
 * exists and cannot open the file. Twenty-eight blank images each time.
 */
