...
| **开着的抽屉里、折叠线以下** | 抽屉开了,tag 查得到,点了仍没反应 | 开抽屉**之后**还要 `performScrollTo()` |
...
`nav:[classic,drawer]` 矩阵存在的意义。

第五个变体紧接着第四个出现,而且是我自己制造的:`goTo()` 的经典分支保留了
`performScrollTo()`,抽屉分支漏了。抽屉里十个条目本身也是纵向可滚的,第十项
(画廊)在 320×640 上就在折叠线以下——**"打开抽屉"和"够得着抽屉里的东西"
是两件事**。十个目的地里只有最后一个能暴露它。

写设备测试时:**查到 tag 只说明它被组合了,要交互或断言可见,先打开、再滚过去。**
...
三个实现细节:class 文件里单个字符串字面量上限 65535 字节,所以按 30000 字符
切块;**块必须在运行时拼**,因为编译器会把常量 `+` 折回成一个字面量,上限照样撞;
**JSON 必须按 Kotlin 规则转义**——`\`、`"`,还有会起模板的 `$`。第一版一个都没转,
生成的文件每一行都是 `Syntax error: Expecting a top level declaration`。生成器现在
把字面量再解析回来跟原 JSON 比对,这类错误不必等 Kotlin 编译器告诉你。
...
**真机安装**：2026-08-25 由用户实测，装的是**编译版**，安装并可用。

这一条正好补上 Cython 那条路上最后一个只能靠推理的环节。干净的交叉编译、
标签正确的 wheel、模拟器上全绿的 41 条断言，**都不等于**那 58 个 `.so` 在真
设备上 load 得起来——尤其在 Python 3.11+ 上，Cython 生成的 C 会用到内部头文件
（本项目锁在 3.10，所以补丁号差一点无所谓，见「版本对齐」）。现在这条不是推
理，是量到的。
