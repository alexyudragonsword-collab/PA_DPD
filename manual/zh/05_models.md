...
- **RLS**(O(N²)/块):每块显式求逆协方差,收敛与条件数无关,**一个
  块**就到最小二乘底(EVM −52 dB)并稳住。
- **白化 NLMS**(一次性 O(N²),之后 O(N)/样本):上电用暖机块的协方差
  做一次 Cholesky 白化,之后在去相关坐标里跑普通 NLMS——**"摊销版
  RLS"**,收敛和 RLS 一样快,稳态甚至更低。
- **APA(仿射投影,K=4,O(N·K))**:在最近 K 个样本窗上去相关(小型
  RLS),几块内爬到接近 RLS——**连续可调**的经典中间档(K=1 即 NLMS,
  K 越大越靠近 RLS)。
- **NLMS**(O(N)):活着但爬得慢、抖,停在比 RLS 高 6–12 dB 处(病态
  方向几乎不动)。
- **普通 LMS**(O(N)):第一个块就发散到 NaN。
...
**怎么选**:小 N + 慢漂移(本项目)→ 低更新率块 RLS 或白化 NLMS 都
近乎免费;大 N / 神经 / 要上硬件 → QR-RLS(要精度、定点数值稳)或
APA / 变换域 LMS(要低成本)。
