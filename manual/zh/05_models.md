...
**时常数辨识(表征驱动的状态配置)**:`StateConditionedSpline` 的
`state_alphas` 不再需要拍脑袋——`padpd.gain_modulation.
identify_gain_modulation(pa, fs)` 执行经典阶跃响应实验(恒包络探针:
满功率短脉冲标定 → 低功率长稳定 → 阶跃升 → 阶跃降),对**复增益**
轨迹(幅度垂降 + AM-PM 漂移联合)做 1..N 极点多指数拟合,时间分箱
平均消采样噪声、段首保护窗剔除电气瞬态(否则静态 PA 会被 FIR 瞬态
误报)。极点数按残差 >10% 改善准则取舍;加热/冷却分开拟合,时常数
比远离 1 提示陷阱/偏置迟滞。在已知真值(5/30 µs)的自热 DUT 上辨识
出 4.9/29.1 µs(权重 0.58/0.42 vs 真值 0.6/0.4);用辨识 α 配置的
状态模型与真值配置差距 <1 dB。探针幅度必须落在压缩区
(单位 RMS 信号默认 a_hi=1.5)——小信号探针对增益调制几乎全盲。

**C-IM3(counter-IM3)与相位谐波分支**:直变发射机里混频器 LO 三次
谐波把基带搬到 3LO-BB(边带反转 → 共轭),经 PA 三阶互调落回
LO-3BB;镜像与主信号的 PA 互调(2(LO-BB)-(LO+BB))也落在同一点。
两种机制的基带等效都是 **conj(x)³**——相位谐波 e^{-j3φ},任何
x·f(|x|) 或 conj(x)·f(|x|) 基在结构上都表示不了。
`SplineMemoryPolynomial(cim3=True)` 追加 conj(x)³·B_j(|x|) 分支族,
`dc_term=True` 加 LO 泄漏常数列;`TxFrontEndPA` 是配套 DUT(镜像 +
LO 泄漏 + 校准 dBc 的 C-IM3 注入)。嵌套消融(-32 dBc C-IM3):
x+共轭+DC 模型钉在 -31.6 dB,加 conj³ 后 **-46.1 dB(+14.5 dB)**。
部署链全程携带相位阶与 DC(LUT 求值、JSON、RTL:conj³ = 一次复数
立方,分支功率标度用左移对齐,iverilog 位真 0 错误)。
**诚实边界(专用抵消环实验结论)**:conj³ 分支服务于**建模/观测
方向**(+14.5 dB);TX 侧闭环抵消我们系统性尝试了五种线性 LS 架构
(ILA 直接带 conj³ 列、DPD 后 u 域注入、DPD 前 x 域注入、交替
"估杂散→清洗观测→重拟合主 DPD"两个记账变体),全部停在 +2~3 dB:
根因是**两环耦合**——(1) 杂散污染主 DPD 的 ILA 估计(无 conj³ 列的
后逆被迫用 x/x* 列吸收杂散,系数偏差与杂散同级且训练拍特定);
(2) 注入项经未线性化的裸 PA 传输,杂散能量集中的峰值区局部复增益
偏离全局 g 最远,环增益误差最大;(3) 交替方案中重拟合的后逆会把
注入"学进去",部署时双重抵消导致不稳。第六个实验是**直接学习**
(`padpd.dpd.direct_learn_spline_dpd`):把 -48 dB 级的 M2 样条模型
作为可微代理,用 torch 对预失真器系数整体做梯度优化(样条基的可微
torch 孪生与 numpy 逐位一致),绕开 ILA 复制假设、梯度天然感知局部
增益。优化器顺滑收敛,留出集比 ILA-M2 再 +1.0 dB、比纯共轭 DPD
+3.6 dB——但**代理内部的级联极限也停在 ~-35 dB**:精确梯度下瓶颈
是预失真器**结构**而非算法——抵消 conj³ 杂散的注入项经过三阶 PA 又
生成更高阶相位谐波,截断在 3 阶的基无法追赶(无穷塔)。工程结论:
DSP 侧对 PA 后 C-IM3 的抑制存在结构天花板(本链路 ~3-4 dB),越过
它要靠混频器硬件侧(谐波抑制混频/LO 占空比调节)而不是更聪明的
DPD。

**QMC 专用抵消环(镜像 + LO 泄漏)**:与 PA 后杂散相反,IQ 失衡与
LO 泄漏发生在 **PA 之前**——`v = a·z + b·z* + c` 有精确代数预逆
(`QMCCorrector.precorrect`),`F(precorrect(w)) = w` 恒等成立,
**没有结构天花板**。工业实践正是把它从 DPD 里拆成独立慢环(QMC /
LO 校准):闭环每拍从残差的 conj(x) 与 DC 投影(经链路增益 g 折算)
修正 (b, c),收敛是几何的——实测每轮镜像残差降约 19 dB,三轮到
**-88.5 dBc**,DC < -70 dBc,且估出的 b 与 DUT 真值 `pa.b/pa.a` 偏差
< 2e-3(IRR 估计与真值差 < 1 dB)。分工收益:QMC 收编镜像/DC 后,
DPD 基可退回**纯相位等变**(去掉 conjugate/dc_term 列,系数减半)——
40 系数纯 x 基 DPD + QMC 达到 -50.8 dB,不劣于 81 系数
conj+dc 宽基 DPD 的 -50.4 dB。每个环各管一个物理机制,LUT/RTL 侧
也无需承载共轭分支。见 `padpd.dpd.QMCCorrector` 与 `tests/test_qmc.py`。

**观测路径去嵌(RX de-embedding 环)**:DPD 从 TX→耦合器→RX 环回
采集自适应,观测路径的每一项损伤(时延、CFO、相位漂移、RX IQ 失衡、
LO 泄漏、噪声)都会被**学进 DPD 系数**——辨识等效于把 RX 逆级联进
预失真器,发射信号替观测路径买单。`padpd.data.ObservationDeembedder`
以已知发送参考为基准,逐次估计并反演观测路径:**顺序有讲究**——先
整数时延对齐(纯相关峰,对 CFO 鲁棒;分数时延的互谱相位斜率估计在
8 kHz CFO 下会偏 1 个样本以上)→ 分段相关相位斜率加权 LS 估 CFO →
`align_delay` 分数时延精配 → CPE 式分段共相漂移(只去均值附近的
漂移,均相位留给增益项)→ widely-linear IQ+DC+增益一步反演(迭代
两次消旋转残差)。PA 自身的相位等变失真对所有校正项投影为零,DPD
要学的内容原样通过;PA 自己的 FIR 群时延会被吸收为一致的参考面偏移
(~0.2 样本,无害,端到端 EVM 需按真实接收机先做定时同步再解调)。
实测(0.3 dB/3° IQ、-40 dBc LO、8 kHz CFO、1° 相噪、7.3 样本时延、
45 dB SNR 的环回):CFO 估计 8.01 kHz、漂移 rms 0.98°;干净环回
DPD EVM -53.5 dB,**不去嵌直接自适应 +3.6 dB(彻底失效)**,经去嵌
自适应 **-48.4 dB**——距干净参考 ~5 dB(残余为相噪快分量与 45 dB
噪声地板)。`wrap(pa, channel, fs)` 把整条链包成 ILA/AdaptiveDPD 可
直接使用的可调用对象。

两项损伤**单采集盲估不可辨识**,用专用标定采集补上(与产品做法
一致):

- **带内 FIR 纹波 + RX 自身 IQ 失衡**(`calibrate_rx_path`):用一次
  **旁路 PA** 的标定采集(已知信号只过观测接收机)估计 RX 的
  widely-linear 响应与 LS 逆 FIR 均衡器,之后每次 `process` 自动
  施加。只有旁路采集能把 RX 的线性响应与 PA 的分开——盲均衡会把 PA
  的线性记忆从观测里洗掉、藏住 DPD 该学的内容;逆 FIR 用"向直通
  正则化"(数据只激励占用带,带外无约束的 LS 逆会放大 PA 失真肩带,
  δ 先验把无数据处钉在全通)。在此标定 RX IQ(而非逐采集盲估)还让
  **TX 镜像保留在校正后的观测里**——QMC 环要看的正是它。实测
  (1 dB 纹波 + 2 ns 群时延纹波):盲估 -30.7 → 标定后 **-35.9 dB**
  (相噪快分量地板)。
- **RX 三阶非线性**(`calibrate_rx_im3`):同一驱动、RX 输入衰减器
  步进两次采集——PA 失真两次相同,RX 立方项随衰减平方缩放,对齐
  尺度后作差即把它分离出来(盲单采集拟合中 PA 立方与 RX 立方回归子
  近共线,无法分辨)。估出 κ 后 `process` 减去 κ·y|y|²(一阶逆)。
  仿真侧 `LoopbackChannel(rx_im3_freeze=True)` 提供物理化(电平
  相关)的 RX IM3。实测(-28 dBc):κ̂ 与真值经链路增益平方一致,
  盲估 -31.8 → 标定后 **-35.9 dB**;纹波 + IM3 同时开:-29.0 →
  **-35.9 dB**。

**三环联合运行(系统实验)**:三个环各自验证过,产品形态要求它们
**同时**闭环、且 PA 在脚下漂移。`padpd.three_loop.run_three_loop`
(脚本 `scripts/run_three_loop_demo.py`)把漂移 PA 放进 TX 前端
(镜像 + LO 泄漏),观测经污染环回(时延/CFO/相噪/RX IQ/纹波/
噪声),每块依次:去嵌观测 → QMC 从残差修正 (b,c) → RLS DPD 绕
QMC 校正后的前端更新。分工使耦合稳定:去嵌器的 RX 级来自旁路标定
(TX 镜像存活给 QMC 看),QMC 收编镜像/DC(DPD 基保持纯相位等变),
DPD 管 PA 非线性与漂移(相位等变失真对其他环的估计项投影为零)。
同一漂移轨迹跑三种配置(12 块,冷→热):

![三环联合:逐块在空口 EVM 与 QMC 残差](../assets/three_loop_demo.png)

- **原始环回自适应**:全程 **+33 dB**(彻底失效)——观测不去嵌,
  自适应学到的全是观测路径;
- **仅去嵌**:跟踪漂移但被 TX 镜像钉在 **-29~-30 dB**(IRR≈30);
- **三环**:块 1 就到 **-41.9 dB**(QMC 一拍收敛),满漂移(热)
  **-35.1 dB**,全程领先仅去嵌 6~12 dB;镜像残差漂移中持续压在
  **-60~-66 dBc**,QMC 的 IRR 估计 30.2 dB(真值 30.1)。热端与
  裸 PA(无前端、完美观测)的自适应地板差距仅 ~1.5-5 dB——整套
  前端 + 污染观测的代价。静态对照(漂移=0)下三环稳在 -36~-37,
  无耦合发散。

**GUI 入口**:PA 建模页合成 DUT 增加「前端」损伤选项(iq / iq+lo /
iq+lo+cim3,配套 Spline-MP-WL / Spline-MP-CIM3 模型);DPD 实验室
页底部「前端三环」面板一键跑三配置对比曲线并注册 run。
