...
![在线 DPD 估计器对比:RLS vs LMS/NLMS](assets/lms_vs_rls.png)

RLS **一个块**就到最小二乘底(EVM −52 dB)并稳住;归一化 NLMS 收敛
慢且抖动、停在比 RLS 高约 6–12 dB 处(病态方向几乎不动);去掉功率
归一化的普通 LMS 第一个块就发散到 NaN。原因:RLS 每块**显式求逆**
协方差,收敛与条件数无关;梯度法的收敛速度正比于条件数(约 1e10),
稳定又要求步长 `< 2/λ_max`,两头受制。产品要"一个块收敛、绝不 NaN"
的确定性,只能用 RLS。
