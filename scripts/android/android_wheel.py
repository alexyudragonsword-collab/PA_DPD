...
----------------------------
Vendored from the ``python-android-apk`` skill so CI can run it; the skill
itself is not part of this repository.  See ``scripts/android/README.md``.
One option was added here, ``--strip-requires``, because padpd's Android build
deliberately installs versions that ``pyproject.toml`` forbids -- see that
option's help and the pip block in ``android/app/build.gradle``.
"""
...
def assemble(package_name: str, tree: Path, dist_info: Path,
             tag: str, outdir: Path, strip_requires: bool = False) -> Path:
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        for path in sorted(dist_info.rglob("*")):
            ...
            if path.name == "METADATA" and strip_requires:
                # Only the Requires-Dist lines go; everything else in
                # METADATA still comes from pyproject.toml, so the name,
                # version and licence cannot drift. See --strip-requires.
                data = ("\n".join(
                    ln for ln in data.decode().splitlines()
...
def main() -> int:
    ...
    ap.add_argument("--strip-requires", action="store_true",
                    help="drop Requires-Dist from the wheel's METADATA. For a "
                         "build whose dependency versions are decided "
                         "elsewhere: Chaquopy's own pip block pins the numpy "
                         "and scipy its repository actually carries, which "
                         "here are BELOW the floors pyproject.toml declares, "
                         "and installs no matplotlib at all. Left in, those "
                         "lines make pip re-litigate a resolution that has "
                         "already been made and fail the build")
    ...
    whl = assemble(package_name, tree, wheel_metadata(project, work),
                   tag, outdir, strip_requires=args.strip_requires)
